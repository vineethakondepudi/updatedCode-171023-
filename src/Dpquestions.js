...
const initialQuestions = questionCategories.map((category, index) => ({
    ...
    questions: category.questions.map((question, questionIndex) => {
      ...
      if (category.name === 'Availability on mTalk') {
        if (questionIndex === 0) {
          initialRating = 0;
      ...
      else if (category.name === 'JavaScript interviews') {
        if (questionIndex === 0) {
          initialRating = 0;
          initialQuantityTarget = 0;
          initialQuantityAchieved = 0; // Add this line
        ...
        else if (category.name === 'Support other projects in DP') {
            if (questionIndex === 0) {
              initialRating = 0;
...
const postDataToApi = (questions) => {
    ...
    axios.post('http://172.17.15.249:4000/DigitalPractiesKPIPost', dataToPost)
        .then(response => {
