...
const initialQuestions = questionCategories.map((category, index) => ({
    ...
    questions: category.questions.map((question, questionIndex) => {
      ...
      if (category.name === 'Code quality assurance per project') {
        if (questionIndex === 0) {
          initialRating = 0;
      ...
      } else if (category.name === 'Deliverables - Over all projects') {
        if (questionIndex === 0) {
          initialRating = 0;
      ...
      else if (category.name === 'Client Satisfaction - Per project') {
        if (questionIndex === 0) {
          initialRating = 0;
          initialQuantityTarget = 4;
          initialQuantityAchieved = 0; // Add this line
        ...
        else if (category.name === 'Client Escalations - Over all projects') {
            if (questionIndex === 0) {
              initialRating = 0;
              initialQuantityTarget = 0;
              initialQuantityAchieved = 0; // Add this line
...
const postDataToApi = (questions) => {
    ...
    axios.post('http://172.17.15.249:4000/BusinessOperatorKPIPost', dataToPost)
        .then(response => {
